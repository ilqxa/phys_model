...
class BaseUnit:
    ...
    def __mul__(self, other:Union[BaseUnit, float])->BaseUnit:
        if type(other) is BaseUnit:
            mark = f'{self.mark}*{other.mark}'
            name = f'{self.name} {other.name}'
            expression = partial(lambda a, b: a.expression() * b.expression(), self, other)
            return BaseUnit(mark=mark, name=name, expression=expression)
    ...
    def __pow__(self, other:Union[BaseUnit, float])->BaseUnit:
        if type(other) is BaseUnit:
            mark = f'{self.mark}**{other.mark}'
    ...
    def __truediv__(self, other:Union[BaseUnit, float])->BaseUnit:
        if type(other) is BaseUnit:
            mark = f'({self.mark})/({other.mark})'
